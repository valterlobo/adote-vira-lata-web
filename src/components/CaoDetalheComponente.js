import React from "react";
import { ethers } from "ethers";
import Metamask from "./Metamask";




const CaoDetalheComponente = (props) => {

  const { src, desc, nome, porte, idade, sexo, cuidados } = props.item;  
  
  return (
    <>
      <div className="detail section-title mt-5 mb-5">
        <div className="container text-white">
          <div className="row align-items-center">
            <div className="col-md-7 mx-auto">
              <div className="services-title mb-5 mt-5">
                <h2 className="title-font title-font-size">{nome}</h2>
              </div>
            </div>
          </div>
          <div className="row">
            <div className="col-md-9">
              <div className="detail-image-box mb-5">
                <img src={src} alt="detail" className="detail-image" />
              </div>
            </div>
            <div className="col-md-3">
              <div className="detail-info text-left title-font-2">
                <h2 className="work-title">{nome}</h2>
                <p className="description title-text mt-4 mb-4 ">{desc}</p>
                <hr />
                <h3 className="work-info mb-3">Info</h3>
                <div className="work-item-category mb-3">
                  <span className="category-title pr-3">Porte:</span>
                  <span className="work-category">{porte}</span>
                </div>
                <div className="work-item-category mb-3">
                  <span className="category-title pr-3">Idade:</span>
                  <span className="work-category">{idade}</span>
                </div>
                <div className="work-item-category mb-3">
                  <span className="category-title pr-3">Sexo:</span>
                  <span className="work-category">{sexo}</span>
                </div>
                <div className="work-item-category mb-3">
                  <span className="category-title pr-3">Cuidados:</span>
                  <span className="work-category">{cuidados.join(" | ")}</span>
                </div>

                <div className="detail-button mb-3">
                    <Metamask item={props.item}/>
                </div>

                <div className="detail-button mb-3">
                  <div className="text-white mb-3">
                      <button className="btn text-white">Adotar</button>
                  </div>
                </div>
              </div>
            </div>
          </div>  
        </div>
      </div>
    </>
  );
};

export default CaoDetalheComponente;
